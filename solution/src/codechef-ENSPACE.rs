fn main() {
	...
	for _test_case in 1..=number_of_test_cases {
		...
		let capacity: i32 = iterable.next().unwrap().parse().unwrap();
		let smaller_files: i32 = iterable.next().unwrap().parse().unwrap();
		let larger_files: i32 = iterable.next().unwrap().parse().unwrap();

		println!("{}", if capacity >= smaller_files +(larger_files * 2) {"YES"} else {"NO"})

fn main() {
	...
	for _test_case in 1..=number_of_test_cases {
		...
		let chef_earnings: i32 = iterable.next().unwrap().parse().unwrap();
		let chefina_earnings: i32 = iterable.next().unwrap().parse().unwrap();

		println!("{}", chefina_earnings - chef_earnings)
